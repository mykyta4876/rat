...
namespace server.Classes
{
    public abstract class Common
    {
        ...
        protected static bool SendMessagesBlocking(NetworkStream stream, byte[][] messages)
        {
            ...
            {
                Console.WriteLine("Send: stream.Write exception: " + exception);
                return false;
            }
        }
        ...
        protected static bool ReadMessageBlocking(NetworkStream stream, int MaxMessageSize, out byte[] content)
        {
            ...
            Console.WriteLine("ReadMessageBlocking: possible allocation attack with a header of: " + size + " bytes.");
            return false;
        }
            ...
            int MaxMessageSize)
        {
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    if (receiveQueue.Count > messageQueueSizeWarning)
                    {
                        ...
                        {
                            Console.WriteLine("ReceiveLoop: messageQueue is getting big(" + receiveQueue.Count +
            ...
            {
                Console.WriteLine("ReceiveLoop: finished receive function for connectionId=" + connectionId + " reason: " +
                             exception);
            }
            ...
            ManualResetEvent sendPending)
        {
            ...
            {
                Console.WriteLine("SendLoop Exception: connectionId=" + connectionId + " reason: " + exception);
            }
        }
    }
}