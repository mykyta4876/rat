...
namespace server.Classes
{
    ...
    public class Client : Common
    {
        ...
        private void ReceiveThreadFunction(string ip, int port)
        {
            ...
            {
                Console.WriteLine("Client Recv: failed to connect to ip=" + ip + " port=" + port + " reason=" + exception);

                receiveQueue.Enqueue(new Message(0, EventType.Disconnected, null));
            }
            catch (Exception exception)
            {
                Console.WriteLine("Client Recv Exception: " + exception);
            }
        ...
        public bool Send(byte[] data)
        {
            ...
            Console.WriteLine("Client.Send: not connected!");
            return false;
        }
    }
}