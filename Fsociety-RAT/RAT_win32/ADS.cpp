...
ADS::ADS()
{
	...
	{
#ifndef LOG_OFF
		std::cout << "ProgramPath=" << Tools::GetThisProgramPath() << "\n\n";
		std::cout << "CurrentUserDir=" << this->currentUserDir << "\n\n";
		std::cout << "HIDE_NAME=" << HIDE_NAME << "\n\n";
...
BOOL ADS::CreateADS(const std::string& source_path, const std::string& destination_path, const std::string& stream_name)
{
	if (Tools::FileExists(destination_path) || Tools::dirExists(destination_path)) // check if the destination to put the stream file on is exists
		if (Tools::FileExists(source_path)) // Check if the file to copy is exists
			...
			{
#ifndef LOG_OFF
				std::cout << "ADS!CreateADS: Stream file already exists\n\n";
				return ADS_ALREADY_EXISTS;
#endif // LOG_OFF
			}
		else
		{
#ifndef LOG_OFF
			std::cout << "ADS!CreateADS: File not found\n\n";
#endif // LOG_OFF
		}
	else
	{
#ifndef LOG_OFF
		std::cout << "ADS!CreateADS: Directory not found\n\n";
#endif // LOG_OFF
	}
